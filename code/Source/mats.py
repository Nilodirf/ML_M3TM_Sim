...
class SimMaterials:
    ...
    def __init__(self, name, tdeb, cp_max, cp_method, kappap, ce_gamma=0., kappae=0., gep=0., vat=0., spin=0., tc=0., muat=0., asf=0.):
        # Input:
    ...
    def create_cp_T(self):
        ...
        if self.cp_method == 'Debye':
            t_grid = np.linspace(1, 4 * self.tdeb, 4000)

            # Calculate the Debye heat capacity
            cp_t_grid = self.debye_heat_capacity(t_grid)
            return t_grid, list(cp_t_grid)
    ...
    def debye_heat_capacity(self, T, num_points=1000):
        ...
        for i, yi in enumerate(t_red):
            ...
            heat_capacities[i] = self.cp_max * D_yi
