...
bounds = [(400, 600), (0.15, 0.35), (8, 10), (1e-3, 1e-1)]

therm_time_initial = (bounds[0][0]+bounds[0][1])/2
gep_initial = (bounds[1][0]+bounds[1][1])/2
te_scaling_initial = (bounds[2][0]+bounds[2][1])/2
power_initial = (bounds[3][0]+bounds[3][1])/2

p0 = [therm_time_initial, gep_initial, te_scaling_initial, power_initial]


####### Simulation to fit:
def fit_te_to_exp(exp_delay, therm_time_test, gep_test, te_scaling_test, power_test):
    ...
    pulse = SimPulse(sample=sample, method='LB', pulse_width=60e-15, fluence=power_test, delay=1.5e-12, therm_time=therm_time_test*1e-15)
...
# Create a laser pulse with the desired parameters. (Fluence in mJ/cm^2)
pulse = SimPulse(sample=sample, method='LB', pulse_width=25.5e-15, fluence=power_fit, delay=1e-12, therm_time=therm_time_fit*1e-15)
