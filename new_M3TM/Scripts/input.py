...
bp = SimMaterials(name='BP', pen_dep=16e-9, tdeb=370, dz=2e-9, vat=1e-28, ce_gamma=107, cp_max=2.17e6, kappap=6.5,
...
sample.add_layers(material=bp, layers=150, kappap_int=0.6, kappae_int='av')

# Create a laser pulse with the desired parameters. (Fluence in mJ/cm^2)
pulse = SimPulse(sample=sample, pulse_width=60e-15, fluence=0.32, delay=1e-12)

# Initialize the simulation with starting temperature and final time, the solver to be used and the maximum timestep:
sim = SimDynamics(sample=sample, pulse=pulse, end_time=10e-12, ini_temp=6., solver='RK23', max_step=1e-13)
